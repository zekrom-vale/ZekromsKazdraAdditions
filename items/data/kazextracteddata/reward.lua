...
function getLevel()
	local level = config.getParameter("treasure.level") or 1
	local maxlevel = config.getParameter("treasure.maxlevel") or level
	local factor = config.getParameter("treasure.factor") or 2
	if maxlevel == level then
		return level
	end
		
	local rez = math.floor(
		math.random(level^factor, (maxlevel)^factor)^(1/factor)	+ 0.5
	)
	return maxlevel - rez + level
end
